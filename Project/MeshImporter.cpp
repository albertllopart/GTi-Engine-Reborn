...
bool MeshImporter::Import(const aiMesh* aimesh, std::string output_file)
{
	...
	mesh->mesh->mesh->num_vertex = aimesh->mNumVertices;
	mesh->mesh->mesh->vertex = new float[mesh->mesh->mesh->num_vertex * 3];
	memcpy(mesh->mesh->mesh->vertex, aimesh->mVertices, sizeof(float) * mesh->mesh->mesh->num_vertex * 3);

	if (aimesh->HasFaces())
	{
		mesh->mesh->mesh->num_index = aimesh->mNumFaces * 3;
		...
		for (int i = 0; i < aimesh->mNumFaces; i++)
		{
			...
			{
				memcpy(&mesh->mesh->mesh->index[i * 3], aimesh->mFaces[i].mIndices, 3 * sizeof(uint));
			}
		}
	}
	...
	if (aimesh->HasNormals())
	{
		...
		memcpy(mesh->mesh->mesh->normals, aimesh->mNormals, sizeof(float) * mesh->mesh->mesh->num_vertex * 3);
	}
	...
	if (aimesh->HasTextureCoords(0))
	{
		...
		{
			memcpy(&mesh->mesh->mesh->texCoords[i * 2], &aimesh->mTextureCoords[0][i].x, sizeof(float));
			memcpy(&mesh->mesh->mesh->texCoords[(i * 2) + 1], &aimesh->mTextureCoords[0][i].y, sizeof(float));
		}
	}

	//calculate memory needed
	
	uint ranges[4] = { mesh->mesh->mesh->num_index, mesh->mesh->mesh->num_vertex, 0, 0 };
	if (aimesh->HasNormals())
	{
		ranges[2] = mesh->mesh->mesh->num_vertex;
	}
	if (aimesh->HasTextureCoords(0))
	{
		ranges[3] = mesh->mesh->mesh->num_vertex;
	}

	float size = sizeof(ranges);
	size += sizeof(uint) * mesh->mesh->mesh->num_index;
	size += sizeof(float) * mesh->mesh->mesh->num_vertex * 3;
	...
	{
		size += sizeof(float) * mesh->mesh->mesh->num_vertex * 2;
	}
	...
	memcpy(pointer, mesh->mesh->mesh->index, sizeof(uint) * mesh->mesh->mesh->num_index);
	pointer += bytes;

	//vertices
	bytes = sizeof(float) * mesh->mesh->mesh->num_vertex * 3;
	memcpy(pointer, mesh->mesh->mesh->vertex, sizeof(float) * 3 * mesh->mesh->mesh->num_vertex);
	pointer += bytes;
	...
	{
		bytes = sizeof(float) * mesh->mesh->mesh->num_vertex * 3;
		memcpy(pointer, mesh->mesh->mesh->normals, sizeof(float) * 3 * mesh->mesh->mesh->num_vertex);
		pointer += bytes;
	}
	...
	if (mesh->mesh->mesh->texCoords != nullptr)
	{
		bytes = sizeof(float) * mesh->mesh->mesh->num_vertex * 2;
